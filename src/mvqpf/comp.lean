...
lemma map_mk (x : F $ λ i, G i α) : f <$$> comp.mk x = comp.mk ((λ i (x : G i α), f <$$> x) <$$> x) := rfl

lemma get_map (x : comp F G α) : comp.get (f <$$> x) = (λ i (x : G i α), f <$$> x) <$$> comp.get x := rfl
...
{ P := mvpfunctor.comp (P F) (λ i, P $ G i),
  ...
  repr' := λ α,  mvpfunctor.comp.mk ∘ repr ∘ map (λ i, (repr : G i α → (λ (i : fin n), apply (P (G i)) α) i)) ∘ comp.get,
  abs_repr' := by { intros, simp [(∘),mv_map_map,(⊚)] },
  abs_map := by { intros, simp [(∘)], rw [← abs_map],
                  simp [mv_id_map,(⊚),map_mk,mvpfunctor.comp.get_map,abs_map,mv_map_map] } }
